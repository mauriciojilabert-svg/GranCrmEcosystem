export interface GranCrmSession {
  ...
  email: string;
  nombre: string;
  // El host puede enviar valores nuevos (admin_ti/admin_cuenta/supervisor/agente)
  // o viejos (sa/admin/ejecutivo). App.tsx los normaliza al canónico viejo.
  rol: 'sa' | 'admin' | 'ejecutivo' | 'admin_ti' | 'admin_cuenta' | 'supervisor' | 'agente';
  tenant_id: string;
  apps: number[];
